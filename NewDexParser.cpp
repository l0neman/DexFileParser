...
void DexParser::parse_string_list(const u4 size, const u4 offset)
{
    ...
    for (u4 i = 0; i < string_ids_size; i++)
    {
        ...
        const auto string_item_size = this->string_list_[i].utf16_size;
...
void DexParser::parse_encoded_field(const int offset, encoded_field* p) const
{
    // parse field_idx_diff.
    {
        ...
        parse_uleb128(leb128_buff, p->field_idx_diff);
    }
...
void DexParser::parse_encoded_method(const int offset, encoded_method* p) const
{
    // parse method_idx_diff.
    {
        if (0 != fseek(this->dex_file_, offset, 0))
        {
            printf("#parse_encoded_method - seek file error 1.\n");
            return;
        }
        ...
        if (0 == fread(leb128_buff, sizeof(u1), 5, this->dex_file_))
        {
            printf("#parse_encoded_method - read file error 2.\n");
            return;
        }

        p->method_idx_diff = new uleb128;
        parse_uleb128(leb128_buff, p->method_idx_diff);
    }
...
void DexParser::parse_class_data_list(const u4 size, const u4 offset) const
{
    ...
    auto class_data_list = new class_data_item[size];

    u4 seek_add = 0;
    for (u4 i = 0; i < size; i++)
    {
        printf("seek add: %d\n", seek_add);
        const auto item = &class_data_list[i];
